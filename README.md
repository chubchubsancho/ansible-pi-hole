...
Role-defined defaults live in `defaults/main.yml` and `vars/main.yml`. You can override any of these from your playbook or inventory (playbook variables win over role defaults). All other `pi_hole_*` variables are consumed by `templates/etc/pihole/pihole.toml.j2`; the defaults listed in the sections below match that template’s `| default(...)` filters when the variable is unset.

### `defaults/main.yml`

<!-- renovate: datasource=github-releases packageName=pi-hole/pi-hole -->
* `pi_hole_version` (default: `"v6.4"`) — Git branch, tag, or commit of the Pi-hole repository to check out.

### `vars/main.yml`

* `pi_hole_repo` (default: `"https://github.com/pi-hole/pi-hole.git"`) — Git remote for the Pi-hole install source.
* `pi_hole_prerequisites` (default: `[git]`) — OS packages installed before cloning (see `tasks/*-prerequisites.yml` for family-specific mapping).
* `pi_hole_install_dir` (default: `"/etc/.pihole"`) — Directory where the Pi-hole repo is cloned before `basic-install.sh` runs.

## Define pi-hole configuration (`pihole.toml`)

The following variables render into `/etc/pihole/pihole.toml`. All are optional unless you need non-default behavior.

## Define pi-hole dns configuration

* `pi_hole_dns_upstreams` — (default: `[]`) — Upstream DNS resolvers. List of strings: IP, `hostname`, or `ip#port` (e.g. `127.0.0.1#5053`). When omitted or empty, the template writes an empty upstream list (configure at least one resolver for non-local queries).
...
* `pi_hole_dns_cname_deep_inspect` — (default: `true`) — Enable deep CNAME inspection; disable on very low-end devices if needed.
* `pi_hole_dns_block_esni` — (default: `true`) — Block `_esni.` subdomains by default (ESNI / ECH privacy vs. local filtering trade-offs; see Pi-hole docs).
* `pi_hole_dns_edns0ecs` — (default: `true`) — Use EDNS0 client subnet (ECS) information to recover client IPs behind NAT. See <https://discourse.pi-hole.net/t/support-for-add-subnet-option-from-dnsmasq-ecs-edns0-client-subnet/35940>.
* `pi_hole_dns_ignore_localhost` — (default: `false`) — Hide queries originated from localhost in FTL.
* `pi_hole_dns_show_dnssec` — (default: `true`) — Analyze and show internally generated DNSSEC queries in FTL.
...
* `pi_hole_dns_reply_when_busy` — (default: `"ALLOW"`) — Behavior while the gravity DB is unavailable: `"BLOCK"`, `"ALLOW"`, `"REFUSE"`, or `"DROP"`.
* `pi_hole_dns_block_ttl` — (default: `2`) — TTL in seconds for blocked replies inside FTL (separate from dnsmasq `local-ttl`). Large values can make client-side whitelisting less effective.
* `pi_hole_dns_hosts` — (default: `[]`) — Custom static hosts entries, each string in HOSTS form `"IP HOSTNAME [HOSTNAME ...]"`.
...
* `pi_hole_dns_domain_name` — (default: `"lan"`) — Local DNS domain name (`dns.domain.name` in TOML). Use `""` to disable assigning a domain.
* `pi_hole_dns_domain_local` — (default: `true`) — Treat this domain as purely local (no upstream forwarding unless a matching reverse server is configured).
...
* `pi_hole_dns_port` — (default: `53`) — DNS listen port.
* `pi_hole_dns_localise` — (default: `true`) — dnsmasq-style localisation of replies to the requesting subnet when possible.
* `pi_hole_dns_rev_servers` — (default: `[]` when unset) — “Reverse server” / conditional forwarding entries. Each string: `"<enabled>,<ip-address>[/<prefix-len>],<server>[#<port>][,<domain>]"`. Components:
...
* `pi_hole_dns_cache_upstream_blocked_ttl` — (default: `86400`) — TTL (seconds) for caching upstream-blocked answers; `0` disables caching.
* `pi_hole_dns_cache_rrtype` — (default: `"ANY"`) — RR types covered by the query cache (`dns.cache.rrtype` in TOML).
* `pi_hole_dns_blocking_enabled` — (default: `true`) — Master switch: FTL applies blocklists when `true`.
...
* `pi_hole_dns_blocking_edns` — (default: `"TEXT"`) — EDNS0 metadata on blocked replies. Modes: `"NONE"` (no extra EDNS), `"CODE"` (BLOCKED info-code 15), `"TEXT"` (code plus human-readable reason).
...
* `pi_hole_dns_reply_host_force4` — (default: `false`) — If `true`, use fixed IPv4 (`pi_hole_dns_reply_host_ipv4`) for hostnames such as `pi.hole`, the device hostname, and FQDN variants.
* `pi_hole_dns_reply_host_ipv4` — (default: `""`) — IPv4 address Pi-hole uses for those replies when `force4` is enabled.
* `pi_hole_dns_reply_host_force6` — (default: `false`) — Same as `force4` for AAAA / IPv6 answers.
* `pi_hole_dns_reply_host_ipv6` — (default: `""`) — IPv6 address when `force6` is enabled.
* `pi_hole_dns_blocking_force4` — (default: `false`) — If `true`, use fixed IPv4 (`pi_hole_dns_blocking_ipv4`) in IP blocking mode and for `;reply=` regex actions.
* `pi_hole_dns_blocking_ipv4` — (default: `""`) — IPv4 address used when blocking uses “reply with IP” semantics.
* `pi_hole_dns_blocking_force6` — (default: `false`) — Same as `blocking_force4` for IPv6 blocking replies.
* `pi_hole_dns_blocking_ipv6` — (default: `""`) — IPv6 used when `blocking_force6` is enabled.
...
* `pi_hole_dhcp_netmask` — (default: `""`) — DHCP pool netmask; empty lets Pi-hole derive it from the local interface (required explicitly for relayed DHCP).
* `pi_hole_dhcp_leasetime` — (default: `""`) — Lease duration string (e.g. `"24h"`, `"45m"`, `"7d"`, `"infinite"`). Empty delegates to dnsmasq defaults (typically 1h IPv4 / 1d IPv6 per upstream docs).
...
* `pi_hole_dhcp_ntp_ipv6_address` - (default: "") - IPv6 address to listen on for NTP requests. Possible values are \<valid IPv6 address\> or empty string ("") for wildcard (`::`).
...
* `pi_hole_resolver_macnames` - (default: true) - Control whether FTL should attempt to obtain client names from the network table by MAC address. This can provide hostnames for devices that do not have a hostname or have multiple IP addresses (e.g. IPv4 and IPv6). However, MAC-derived names can be ambiguous (e.g., when a MAC appears for multiple IPs due to a router/NAT or MAC reuse), which may lead to incorrect hostnames being shown. Disabling this option can prevent such issues but may lead to more clients without hostnames.
...
* `pi_hole_database_forcedisk` — (default: `false`) — When `true`, keep query history on disk instead of the default in-memory DB (saves RAM on small systems; slower, especially on SD cards). Uses `files.tmp_db` for the short-term DB path.
...
* `pi_hole_webserver_headers` — (optional) — When set, replaces the entire `headers` array in TOML with this list of `Name: value` strings. When unset, the template emits Pi-hole’s current built-in default header list (see `templates/etc/pihole/pihole.toml.j2`).
* `pi_hole_webserver_advanced_opts` — (optional; default `[]`) — Extra CivetWeb options as `"key=value"` strings appended to the web server config (advanced; invalid values can break the UI).
...
* `pi_hole_webserver_tls_validity` — (default: `47`) — TLS certificate validity window (days) used when Pi-hole generates internal TLS material (`webserver.tls.validity`).
...
* `pi_hole_webserver_interface_boxed` — (default: `true`) — Use the boxed layout in the web UI (`webserver.interface.boxed`).
* `pi_hole_webserver_interface_theme` — (default: `"default-auto"`) — Theme (`webserver.interface.theme`): `"default-auto"`, `"default-light"`, `"default-dark"`, `"default-darker"`, `"high-contrast"`, `"high-contrast-dark"`, `"lcars"`.
...
* `pi_hole_webserver_api_excludedomains` - (default: []) - Array of domains to be excluded from certain API responses (regex):
  * Query Log (/api/queries)
  * Top Domains (/api/stats/top_domains)

  Note that backslashes "\" need to be escaped, i.e. "\\" in this setting. Possible values are array of regular expressions describing domains. Example:
...
* `pi_hole_webserver_api_max_history` — (default: `86400`) — History depth (seconds) returned by the API (max 86400).
* `pi_hole_webserver_api_max_clients` — (default: `10`) — Cap for `/api/history/clients`; `0` returns all clients (can be heavy in large LANs).
...
* `pi_hole_webserver_api_temp_limit` — (default: `60.0`) — Upper temperature bound for the “hot” indicator; interpreted in `pi_hole_webserver_api_temp_unit`.
...
* `pi_hole_files_tmp_db` — (default: `"/etc/pihole/pihole-tmp.db"`) — Short-term SQLite path used when `pi_hole_database_forcedisk` is enabled.
...
* `pi_hole_misc_normalize_cpu` — (default: `true`) — Normalize CPU usage in reports across different core counts (`misc.normalizeCPU`).
* `pi_hole_misc_hide_dnsmasq_warn` — (default: `false`) — Suppress dnsmasq warning lines from logs (`misc.hide_dnsmasq_warn`).
* `pi_hole_misc_hide_connection_error` — (default: `false`) — Hide connection error messages (`misc.hide_connection_error`).
...
* `pi_hole_misc_check_shmem` — (default: `90`) — Shared-memory usage warning threshold (percent); `0` disables the check (`misc.check.shmem`).
...
* `pi_hole_debug_caps` — (default: `false`) — Log Linux capabilities granted to `pihole-FTL` (refresh via `SIGHUP` without full restart).
...
* `pi_hole_debug_timing` — (default: `false`) — Debug timing-related internals (`debug.timing` in TOML).
